 ...
 * @desc    Update customer's medical profile
 * @access  Private (Customer)
 */
router.patch('/profile', protect, isCustomer, async (req, res) => {
    try {
        const { bloodGroup, height, weight, chronicDiseases, allergies } = req.body;
        ...
        // Update fields
        if (bloodGroup !== undefined) customer.bloodGroup = bloodGroup;
        if (height !== undefined) customer.height = height;
        if (weight !== undefined) customer.weight = weight;
        if (chronicDiseases !== undefined) customer.chronicDiseases = chronicDiseases;
